...
```{r purl=FALSE, echo=TRUE, opts.label='fig.wider', fig.height=5}
ggplot(data = diamonds, aes(x = cut, y = price, colour = clarity)) +
  stat_summary(geom = 'pointrange', fun.data = mean_cl_normal) +
  facet_wrap(~ clarity) +
...
Нет оснований для сомнения, что наша выборка взята из генеральной совокупности с распределением, которое отличается от нормального.
...
*At!* Лучше не использовать популярное словосочетание "*достоверно отличается*". 

Достоверные события - это события, вероятность которых равна 1. 
...
$$SE_{\bar{x}_1 - \bar{x}_2} = \sqrt{ \frac{\sigma^2}{n_{1}} + \frac{\sigma^2}{n_{2}}} \approx \sqrt{ \frac{sd^2}{n_{1}} + \frac{sd^2}{n_{2}}}$$

$s^2 = \cfrac{(n_1 - 1)sd^2_1 + (n_2 - 1)sd^2_2}{n_1 + n_2 - 2}$ — это __обобщенная дисперсия__ по двум выборкам.

Результирующая $t$-статистика 

$$t\;= \;\frac{\bar{x}_1 - \bar{x}_2}{\sqrt{SE_1^2 + SE_2^2}}$$

подчиняется $t$-распределению с  $df = n_1 + n_2 - 2$.

--

Осторожно, равенство дисперсий в группах — это часто нереалистичное предположение!
...
$$SE_{\bar{x}_1 - \bar{x}_2} = \sqrt{ \frac{\sigma^2_{1}}{n_{1}} + \frac{\sigma^2_{2}}{n_{2}}} \approx \sqrt{ \frac{sd^2_{1}}{n_{1}} + \frac{sd^2_{2}}{n_{2}}}$$
...
t-тестом Уэлча можно пользоваться, даже если дисперсии равны (это более общее решение).
...
Давайте сравним секрецию тетрагидрокортизона при аденоме и двусторонней гиперплазии надпочечников. Различается ли она?
...
## Результаты параметрического теста
