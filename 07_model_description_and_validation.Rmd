...
---
## Если стохастическая модель адекватна, то что мы можем сказть про остатки?
...
## Чем опасна автокоррляция (и любая другая зависимость наблюдений друг от друга)
...
plot_grid(
  ...
  ggplot(data = mod1_diag, aes(x = x1, y = .stdresid)) + 
    geom_point() + geom_hline(yintercept = 0),
  function(){par(mar = c(4, 4, 1, 1)); qqPlot(mod1, id = FALSE)},
  nrow = 2)
```
...
plot_grid(
  ...
  ggplot(data = mod2_diag, aes(x = x2, y = .stdresid)) + 
    geom_point() + geom_hline(yintercept = 0),
  function(){par(mar = c(4, 4, 1, 1)); qqPlot(mod2, id = FALSE)},
  nrow = 2)
```
