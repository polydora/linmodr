...
author: "Марина Варфоломеева, Вадим Хайтов, Анастасия Лянгузова, Валентина Домашкина"
date: "Осень `r format(Sys.Date(), '%Y')`"
output:
  xaringan::moon_reader:
    self_contained: true
    lib_dir: libs
    css: [ninjutsu, "assets/xaringan-themer.css", "assets/xaringan.css", "assets/scrollable.css"]
    df_print: default
    nature:
...
$$Longevity_{i} = \beta_0 + \beta_1 Thorax_{i} + \beta_{2} I_{isolated} + \beta_{3} I_{one} + \beta_{4} I_{many} + \beta_{5} I_{low} + \\ + Interactions + \varepsilon_{i}$$
...
```{r}
M1_diagn <- fortify(M1)
ggplot(M1_diagn, aes(x = .fitted, y = .stdresid)) + geom_point() + 
...
p_values <- data.frame(p_no_heter = rep(NA, 1000), 
                       p_heter = NA)
...
```{r highlight.output=c(4:5, 11, 24)}
summary(M2_gls)
```
...
.pull-right[
Три группы крыс содержали при разных условиях кормления 64 дня. Каждую крысу взвешивали с определённой периодичностью. 
...
Построить модель, отвечающую на вопрос: изменяется ли характер роста крыс в зависимости от типа диеты?
